...
export function chipStyle(active, accent = 'red') {
  ...
  return {
    ...
    fontSize: '13px',
    fontWeight: '700',
    background: active ? p.background : C.bg,
    border: `1px solid ${active ? p.border : C.borderSoft}`,
    color: active ? p.color : C.textSec,
    boxShadow: active ? `0 8px 18px ${p.background}` : 'none',
    whiteSpace: 'nowrap',
  };
}
