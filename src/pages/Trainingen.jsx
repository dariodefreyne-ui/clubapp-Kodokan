...
function GroepsKaart({ groep, index, actief, onClick, aantalKomend, T, isDark }) {
 ...
 return (
 <button onClick={onClick} style={{ flexShrink: 0, minWidth: '160px', padding: '16px 18px', borderRadius: '14px', cursor: 'pointer', textAlign: 'left', background: actief ? isDark ? T.navyMid : '#fff' : isDark ? 'transparent' : 'rgba(255,255,255,0.5)', border: actief ? `1.5px solid ${accent}` : '1.5px solid transparent', transform: actief ? 'scale(1.03)' : 'scale(1)', boxShadow: actief ? `0 4px 20px rgba(0,0,0,0.2), 0 0 0 1px ${accent}30` : 'none', transition: 'all 0.18s cubic-bezier(.4,0,.2,1)', outline: 'none' }}>
...
function HeroSection({ seizoenLabel, aantalKomend, volgendeTraining, mijnNaam, onNieuw, kanNieuw }) {
 ...
 return (
 <div style={{ background: 'linear-gradient(135deg, #06101A 0%, #0D1B2A 60%, #1B2A3D 100%)', borderRadius: '14px', padding: '20px 24px 18px', marginBottom: '20px', position: 'relative', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.25)' }}>
 ...
 <div><div style={{ fontSize: '11px', fontWeight: '800', color: 'rgba(56,189,248,0.75)', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '4px' }}>Kodokan · {seizoenLabel}</div><h1 style={{ margin: 0, fontSize: 'clamp(20px,4vw,26px)', fontWeight: '900', color: '#EBF2FA', letterSpacing: '-0.5px', lineHeight: 1.1 }}>Trainingsplanning</h1></div>
...
export default function Trainingen() {
 ...
 return (
 <div style={{ background: isDark ? '#06101A' : '#F0F4F8', minHeight: '100vh', color: T.text, paddingBottom: '48px', fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
 ...
 <div style={{ background: isDark ? '#0D1B2A' : T.surface, boxShadow: isDark ? `0 1px 0 ${T.borderSoft}` : '0 1px 0 rgba(0,0,0,0.07)', padding: '0 20px', height: '52px', display: 'flex', alignItems: 'center', gap: '10px', position: 'sticky', top: 0, zIndex: 50 }}>
