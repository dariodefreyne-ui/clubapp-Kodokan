...
function TrainerCheckNu() {
    ...
    try {
      ...
      const interval = setInterval(async () => {
        ...
        if (data?.status === 'klaar') {
          ...
          clearInterval(interval);
          setResultaat({ ok: false, bericht: data.fout || 'Onbekende fout.' });
          setBezig(false);
        } else if (pogingen >= 15) {
          clearInterval(interval);
          setResultaat({
            ok: true,
            bericht: 'Check gestart. Resultaat verschijnt in de logs (duurt max 30 sec).',
          });
          setBezig(false);
        }
      }, 1000);
  ...
  return (
    <div>
      <div style={{ fontSize: '13px', color: '#aaa', marginBottom: '16px' }}>
        Voer de trainer-check nu manueel uit, ongeacht de dag of het tijdstip.
        Trainers zonder lesgever in de ingestelde periode ontvangen meteen een push en/of mail.
      </div>
