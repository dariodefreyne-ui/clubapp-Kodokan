 ...
 *
 * Wijzigingen v4:
 *  - MailImport parser volledig herschreven:
 *      • Mail = één lange string zonder echte newlines
 *      • Splits op tornooipatroon (datum + maandnaam als grens)
 *      • "Ja" vastgeplakt NA tornooinaam → die judoka is ingeschreven
 *      • Meerdere Ja's mogelijk in één mail
 *  - Categorie-logica verfijnd op basis van tornooi doelgroep:
 *      • Doelgroep bepaalt welke categorieën toegelaten zijn
 *      • U11/U13 → alleen U11 en U13 (U14 niet gebruiken)
 *      • U15+ → U15, U18, U21+ (U16 niet gebruiken)
 *      • Als tornooi expliciet U14 of U16 vermeldt → wel gebruiken
 *      • Judoka krijgt meest passende categorie binnen wat tornooi toelaat
 *  - UI: Zoekbalk op judoka (naam) in detailpanel
 *  - UI: Detailpanel tabs → judoka eerst, info tweede
 *  - UI: Beginpagina → judoka-teller prominenter, locatie weg uit kaart
 *  - Waarschuwing in preview als judoka niet past in tornooi-doelgroep
 */
...
// Rangorde van categorieen, laag naar hoog
const CAT_RANGORDE = ['U9', 'U11', 'U13', 'U14', 'U15', 'U16', 'U18', 'U21+'];

// Bepaal de toegelaten categorieën op basis van de doelgroep-string
function parseerToegelatenCategorieen(doelgroep) {
  if (!doelgroep) return null; // null = geen restrictie
  ...
  // Voeg elke expliciet vermelde categorie toe
  for (const cat of CAT_RANGORDE) {
    const escaped = cat.replace('+', '\\+');
    if (new RegExp(`(^|[^0-9])${escaped}([^0-9+]|$)`).test(d)) {
      toegelaten.add(cat);
    }
  }

  // "U15+" of "U15-U21+" = U15, U18, U21+
...
// Geeft { cat, buiten } terug
// buiten=true als de judoka niet echt in de doelgroep past
function berekenCategorie(geboortejaar, tornooidatum, doelgroep = null) {
  ...
  // Zoek dichtstbijzijnde hogere toegelaten categorie
  for (let i = rawIdx; i < CAT_RANGORDE.length; i++) {
    if (toegelaten.includes(CAT_RANGORDE[i])) {
      return { cat: CAT_RANGORDE[i], buiten: false };
    }
  }
  // Zoek dichtstbijzijnde lagere (buiten doelgroep)
  for (let i = rawIdx - 1; i >= 0; i--) {
    if (toegelaten.includes(CAT_RANGORDE[i])) {
      return { cat: CAT_RANGORDE[i], buiten: true };
    }
  }
...
/**
 * Parseert de ruwe Google Form mail-tekst.
 * Formaat: één lange string, alles vastgeplakt.
 * Tornooigrens = "TornooiNaam - dag maandnaam [jaar][Ja]"
 * "Ja" is vastgeplakt aan het einde van de tornooinaam als de judoka ingeschreven is.
 */
function parseerMailTekst(mailTekst) {
  ...
  if (naamMatch) {
    naamJudoka = naamMatch[1].trim();
  }

  // ── 2. Kap footer af ──────────────────────────────────────────────────────
  const footerRe = /gewicht\s*\(|uitschrijven\s+voor/i;
  const footerIdx = mailTekst.search(footerRe);
  const relevantTekst = footerIdx > -1 ? mailTekst.slice(0, footerIdx) : mailTekst;

  // ── 3. Zoek alle tornooiblokken via globale regex ─────────────────────────
  // Patroon: alles vóór "- dag maandnaam [jaar][Ja]"
  // We gebruiken een lookahead om de grenzen te vinden
  const tornooiRe = new RegExp(
    `(.+?)-\\s*(\\d{1,2})\\s+(${MAAND_RE_STR})(?:\\s+(\\d{4}))?(Ja)?(?=-|$|gewicht|uitschrijven)`,
    'gi'
  );

  // Alternatief: split op datumpatroon en verwerk per segment
  // Robuuster: zoek alle posities van "- dag maandnaam"
  ...
  for (let i = 0; i < grenzen.length; i++) {
    ...
    // Naam = tekst vóór de "-datum" grens, na de vorige grens
    ...
    inschrijvingen.push({
      label,
      tornooiNaam: tornooiNaamRaw,
      datum,
      ingeschreven: heeftJa,
      tornooi: null,
      matched: false,
    });
  }
...
function fuzzyMatch(haystack, needle) {
  ...
  const stopwoorden = new Set(['cup', 'van', 'de', 'het', 'voor', 'over', 'en', 'op', 'in', 'bij']);
  const woorden = needle.toLowerCase()
    .replace(/[()[\]/]/g, ' ')
    .split(/\s+/)
    .filter(w => w.length > 2 && !stopwoorden.has(w));
  if (woorden.length === 0) return false;
  const matches = woorden.filter(w => h.includes(w));
  return matches.length >= Math.min(2, woorden.length);
}
...
function Badge({ label, style = {} }) {
  return (
    <span style={{ display:'inline-block', padding:'2px 9px', borderRadius:'999px', fontSize:'11px', fontWeight:'700', letterSpacing:'0.4px', ...style }}>
      {label}
    </span>
  );
}
...
function TournamentCard({ event, isSelected, onClick }) {
  ...
  return (
    ...
    >
      {/* Datum blok */}
      <div style={{
        minWidth:'54px', padding:'12px 8px',
        display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
        borderRight:`1px solid ${C.border}`, background: isSelected ? C.redDim : 'transparent',
      }}>
      ...
      <div style={{ flex:1, padding:'10px 12px', minWidth:0 }}>
        ...
        {/* Locatie niet getoond in kaart (UI spec) */}
      </div>

      {/* Judoka teller — prominenter, alleen tonen als > 0 */}
      {(event._judokaCount || 0) > 0 && (
        <div style={{
          padding:'10px 14px', display:'flex', flexDirection:'column',
          alignItems:'center', justifyContent:'center', minWidth:'50px',
          background:C.redDim, borderLeft:`1px solid ${C.redBord}`,
        }}>
          <span style={{ fontSize:'18px', fontWeight:'800', color:C.red, lineHeight:1 }}>
            {event._judokaCount}
          </span>
...
function DetailPanel({ event, onClose, onUpdate, onDelete }) {
  const [tab, setTab]               = useState('judoka'); // judoka eerst
  ...
  const [judokaSearch, setJudokaSearch] = useState(''); // zoekbalk judoka
  ...
  async function handleAddJudoka() {
    ...
    try {
      ...
      await updateDoc(doc(db, 'events', event.id), { _judokaCount: judoka.length + 1 });
  ...
  async function handleRemoveJudoka(jid) {
    ...
    await updateDoc(doc(db, 'events', event.id), { _judokaCount: Math.max(0, judoka.length - 1) });
  }
  ...
  const inputStyle = {
    ...
    borderRadius:'8px', color:C.text, padding:'9px 12px', fontSize:'14px',
    boxSizing:'border-box', fontFamily:'inherit', outline:'none',
  };

  // Categorie preview voor nieuw toe te voegen judoka
  const catPreview = newJudoka.geboortejaar?.length === 4
    ? berekenCategorie(newJudoka.geboortejaar, event.datum, event.doelgroep)
    : null;

  // Filter + groepeer judoka
  const gefilterdJudoka = judokaSearch.trim()
    ? judoka.filter(j => j.naam?.toLowerCase().includes(judokaSearch.toLowerCase()))
    : judoka;

  const byCategorie = gefilterdJudoka.reduce((acc, j) => {
    ...
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:'14px', display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      ...
      <div style={{ padding:'16px 18px 0', borderBottom:`1px solid ${C.border}`, flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'flex-start', gap:'10px', marginBottom:'12px' }}>
          ...
          <button onClick={onClose} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:'8px', color:C.textSec, fontSize:'16px', cursor:'pointer', padding:'6px 10px', lineHeight:1, flexShrink:0, fontFamily:'inherit' }}>
            ✕
          </button>
        </div>
        ...
        <div style={{ display:'flex', gap:0 }}>
          {[['judoka', `👥 Judoka (${judoka.length})`], ['info', 'ℹ️ Info']].map(([t, l]) => (
            ...
            <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:'10px', padding:'14px', marginBottom:'18px' }}>
              ...
              {catPreview && (
                <div style={{ fontSize:'12px', marginBottom:'8px', display:'flex', alignItems:'center', gap:'6px' }}>
                  ...
                  {catPreview.buiten && (
                    <span style={{ fontSize:'11px', color:C.amber, background:'rgba(245,158,11,0.1)', padding:'1px 6px', borderRadius:'4px', border:'1px solid rgba(245,158,11,0.3)' }}>
                      ⚠ buiten doelgroep tornooi
                    </span>
                  )}
                </div>
              )}
    ...
    try {
      ...
      for (const row of dataRows) {
        ...
        if (rawDate instanceof Date)      dateStr = rawDate.toISOString().slice(0, 10);
          ...
          dateStr = new Date(Math.round((rawDate - 25569) * 86400 * 1000)).toISOString().slice(0, 10);
        }
        ...
        if (match) {
          ...
          batch.set(doc(collection(db, 'events')), { ...data, createdAt: serverTimestamp() });
          added++;
        }
      }
...
function MailImport({ events, onDone }) {
  ...
  function handlePreview() {
    ...
    const metMatch = parsed.inschrijvingen.map(ins => {
      let tornooi = null;
      tornooi = events.find(e => e.datum === ins.datum && fuzzyMatch(e.naam, ins.tornooiNaam));
  ...
  async function handleImport() {
    ...
    for (const ins of preview.inschrijvingen) {
      ...
      try {
        // Fase 3 (niet actief): geboortejaar via ledenlijst
        const { cat } = berekenCategorie(null, ins.tornooi.datum, ins.tornooi.doelgroep);

        const bestaandSnap = await getDocs(query(collection(db, 'events', ins.tornooi.id, 'judoka')));
        const isDubbel = bestaandSnap.docs.some(
          d => d.data().naam?.toLowerCase().trim() === preview.naamJudoka.toLowerCase().trim()
        );
      ...
      {!preview ? (
        ...
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:'10px', padding:'16px' }}>
          ...
          <div style={{ fontSize:'11px', color:C.textSec, textTransform:'uppercase', letterSpacing:'0.6px', marginBottom:'8px' }}>
            Ingeschreven voor ({ingeschrevenIns.length})
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap:'6px', marginBottom:'14px' }}>
            {ingeschrevenIns.length === 0 ? (
              <div style={{ fontSize:'13px', color:C.textMut, padding:'8px 0' }}>Geen inschrijvingen gevonden in de mail.</div>
...
export default function Wedstrijden() {
  ...
  async function handleCreate() {
    ...
    setCreating(true);
    try {
      const ref = await addDoc(collection(db, 'events'), { ...newForm, type:'wedstrijd', _judokaCount:0, createdAt: serverTimestamp() });
  ...
  return (
    <div style={{ color:C.text, fontFamily:"-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Segoe UI', sans-serif", paddingBottom:'40px' }}>
      ...
      <div style={{ marginBottom:'24px' }}>
        ...
        {/* Stats — judoka teller prominent */}
        <div style={{ display:'flex', gap:'10px', marginTop:'16px', flexWrap:'wrap' }}>
          {/* Judoka: groot, rode achtergrond */}
          <div style={{ background:C.redDim, border:`1px solid ${C.redBord}`, borderRadius:'10px', padding:'12px 20px', display:'flex', alignItems:'center', gap:'10px' }}>
            <span style={{ fontSize:'20px' }}>👥</span>
            <div>
              <span style={{ fontWeight:'900', fontSize:'26px', color:C.red, display:'block', lineHeight:1 }}>{stats.judoka}</span>
              <span style={{ fontSize:'11px', color:C.red, opacity:0.8 }}>judoka ingeschreven</span>
            </div>
          </div>
          {[['🏆', stats.total, 'tornooien'], ['📅', stats.upcoming, 'komend']].map(([icon, val, lbl]) => (
      ...
      <div style={{ display:'grid', gridTemplateColumns: selected ? 'minmax(0,1fr) minmax(0,420px)' : '1fr', gap:'20px', alignItems:'start' }}>
            ...
            <>
              {upcomingList.length > 0 && (
                <Section label={`Komende tornooien (${upcomingList.length})`}>
                  {upcomingList.map(e => <TournamentCard key={e.id} event={e} isSelected={selected?.id === e.id} onClick={() => setSelected(selected?.id === e.id ? null : e)} />)}
                </Section>
              )}
              {pastList.length > 0 && (
                <Section label={`Voorbije tornooien (${pastList.length})`} muted>
                  {pastList.map(e => <TournamentCard key={e.id} event={e} isSelected={selected?.id === e.id} onClick={() => setSelected(selected?.id === e.id ? null : e)} />)}
                </Section>
              )}
            </>
          )}
        ...
        {selected && (
          <div style={{ position:'sticky', top:'16px', maxHeight:'85vh', display:'flex', flexDirection:'column', animation:'fadeIn 0.2s ease' }}>
            <DetailPanel event={selected} onClose={() => setSelected(null)} onUpdate={updated => setSelected(updated)} onDelete={() => setSelected(null)} />
          </div>
        )}
      </div>
    </div>
