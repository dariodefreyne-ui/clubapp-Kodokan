...
import { subscribe, runManualCheck, clearLog, berekenVerbindingsStatus } from '../../services/syncMonitor';

const TYPE_KLEUR = { error: C.red, warn: C.orange, sync: C.green, info: C.textMuted };
const TYPE_LABEL = { error: '✗', warn: '⚠', sync: '✓', info: 'ℹ' };

const VERBINDING_UI = {
  bevestigd: { icoon: '🟢', label: 'Online — Firestore bevestigd', kleur: C.green },
  onbevestigd: { icoon: '🟠', label: 'Online — Firestore niet recent bevestigd', kleur: C.orange },
  offline: { icoon: '🔴', label: 'Offline', kleur: C.red },
};
...
export default function SyncDebugPanel({ onClose }) {
  ...
  const verbinding = berekenVerbindingsStatus(snap, now);
  const verbindingUi = VERBINDING_UI[verbinding];
    ...
    <div style={{
      ...
      >
        ...
        <div style={{
          ...
          borderRadius: '10px', marginBottom: '14px', fontSize: '13px', fontWeight: '700',
          background: `${verbindingUi.kleur}22`, color: verbindingUi.kleur,
        }}>
          <span style={{ fontSize: '16px' }}>{verbindingUi.icoon}</span>
          <span>{verbindingUi.label}</span>
        </div>
        ...
        <div style={{ marginBottom: '14px' }}>
          ...
          {rij('Laatste Firestore-sync', geledenTekst(snap.lastSyncAt ? now - snap.lastSyncAt : null), verbindingUi.kleur)}
