...
export default function DetailPanel({ event, inschrijvingenVoorEvent, onClose, onUpdate, onDelete }) {
  ...
  // Begeleider state
  const [coaches, setCoaches]         = useState([]);   // actieve lesgevers
  const [begeleiders, setBegeleiders] = useState([]);   // [{lesgeverId, naam, aanwezig, km, inkom}]
  const [savingBeg, setSavingBeg]     = useState(false);

  // Laad actieve lesgevers eenmalig — 1 read, niet realtime
  useEffect(() => {
    getDocs(collection(db, 'lesgevers')).then(snap => {
      ...
      // Resolve de tijdelijke _defaultUid naar een echte lesgeverId
      setBegeleiders(prev => prev.map(b => {
        if (!b._defaultUid) return b;
        const gekoppeld = lijst.find(l => l.uid === b._defaultUid);
        if (gekoppeld) {
          return { lesgeverId: gekoppeld.id, naam: gekoppeld.naam, aanwezig: true, km: '', inkom: '' };
        }
        // Ingelogde user is geen gekende lesgever — toon toch als noodoplossing
        return { lesgeverId: b._defaultUid, naam: b.naam, aanwezig: true, km: '', inkom: '' };
      }));
    }).catch(console.error);
  }, []);
  ...
  useEffect(() => {
    ...
    setTab('judoka');
    setJudokaSearch('');
    // Init begeleiders vanuit event, of zoek huidig profiel op in lesgevers als default
    const opgeslagen = Array.isArray(event?.begeleiders) ? event.begeleiders : [];
    if (opgeslagen.length === 0 && profiel?.uid) {
      // Zoek de lesgever die gekoppeld is aan de ingelogde user
      // coaches is mogelijk nog niet geladen — we gebruiken een lazy init in de coaches useEffect
      setBegeleiders([{ _defaultUid: profiel.uid, naam: profiel.naam || '', aanwezig: true, km: '', inkom: '' }]);
    } else {
  ...
  // Begeleider helpers — identifier is lesgeverId (stabiel, onafhankelijk van naam of uid)
  function updateBegeleider(lesgeverId, veld, waarde) {
    setBegeleiders(prev => prev.map(b => b.lesgeverId === lesgeverId ? {...b, [veld]: waarde} : b));
  }
  function voegBegeleiderToe(lesgever) {
    if (begeleiders.find(b => b.lesgeverId === lesgever.id)) return;
    setBegeleiders(prev => [...prev, { lesgeverId: lesgever.id, naam: lesgever.naam, aanwezig: true, km: '', inkom: '' }]);
  }
  function verwijderBegeleider(lesgeverId) {
  ...
  return (
    <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:'14px',display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'18px'}}>
        ...
        {tab==='begeleider' && (() => {
          const beschikbaar = coaches.filter(c => !begeleiders.find(b => b.lesgeverId === c.id));
          return (
            <div>
              {/* Begeleiders lijst */}
              ...
              {beschikbaar.length > 0 && (
                <div style={{marginBottom:'14px'}}>
                  ...
                  <select
                    defaultValue=""
                    onChange={e => {
                      const lesgever = coaches.find(c => c.id === e.target.value);
                      if (lesgever) voegBegeleiderToe(lesgever);
                      e.target.value = '';
                    }}
