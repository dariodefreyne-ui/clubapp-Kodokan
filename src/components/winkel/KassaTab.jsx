...
function ProductIcon({ product, size = 42 }) {
    ...
    >
      <div>
        ...
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          ...
          {geenPrijs && (
            <span style={{ background: '#555', color: '#fff', borderRadius: '5px', padding: '2px 6px', fontSize: '10px', fontWeight: '700' }}>
              Geen prijs
            </span>
          )}
        </div>
...
export default function KassaTab({ products, profiel }) {
  ...
  function removeOne(product) {
    setCart(c => c
      .map(i => i.id === product.id ? { ...i, qty: Math.max(0, i.qty - 1) } : i)
      .filter(i => i.qty > 0)
    );
  }
  ...
  function changeQty(id, delta) {
    setCart(c => c
      .map(i => i.id === id ? { ...i, qty: Math.max(0, Math.min(i.maxStock, i.qty + delta)) } : i)
      .filter(i => i.qty > 0)
    );
  }
  ...
  if (success) {
    return (
      <div style={{ minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        ...
        <div style={{ color: '#aaa', fontSize: '16px', marginBottom: '4px' }}>
        ...
        {success.betaalmethode === 'overschrijving' && (
          <div style={{ color: '#f39c12', marginTop: '12px', fontSize: '14px' }}>
            Openstaande schuld geregistreerd
          </div>
        )}
      </div>
    );
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '18px' }}>
          <button
            ...
            style={{ background: 'none', border: 'none', color: '#aaa', fontSize: '26px', cursor: 'pointer', lineHeight: 1 }}
          >
            ←
          </button>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  padding: '20px 10px',
                  borderRadius: '12px',
                  border: `2px solid ${method === m ? '#c0392b' : '#3a3a3a'}`,
                  background: method === m ? 'rgba(192,57,43,0.15)' : '#2d2d2d',
                  color: method === m ? '#fff' : '#bbb',
                  fontSize: '16px',
                  fontWeight: '700',
                  cursor: 'pointer',
                }}
        ...
        <div style={{ marginBottom: '18px' }}>
          ...
          {toonKoppelWaarschuwing && (
            <div style={{ background: 'rgba(243,156,18,0.12)', border: '1px solid #f39c12', color: '#f39c12', borderRadius: '8px', padding: '10px 12px', fontSize: '13px', marginBottom: '12px' }}>
          ...
          {koperId ? (
            <button
              onClick={ontkoppelLid}
              style={{ background: 'rgba(39,174,96,0.15)', border: '1px solid #27ae60', color: '#27ae60', padding: '10px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}
            >
              ...
              {zoekOpen && (
                <>
                  ...
                  {zoekResultaten.length > 0 && (
                    <div style={{ background: '#2d2d2d', border: '1px solid #3a3a3a', borderRadius: '8px', overflow: 'hidden' }}>
                      {zoekResultaten.map(u => (
                        <button
                          key={u.id}
                          onClick={() => {
                            setKoperId(u.id);
                            setKoperNaam(u.naam || u.name || koperNaam);
                            setZoekterm('');
                            setZoekResultaten([]);
                            setZoekOpen(false);
                          }}
        ...
        <div style={{ background: '#2d2d2d', borderRadius: '12px', padding: '14px', marginBottom: '18px' }}>
          {cart.map(i => (
            <div key={i.id} style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', padding: '7px 0', fontSize: '14px' }}>
              <span>{i.name} {i.variant} ×{i.qty}</span>
              <strong>{fmtBedrag(i.price * i.qty)}</strong>
            </div>
          ))}
          ...
          {method && (
            <div style={{ color: '#aaa', fontSize: '13px', marginTop: '8px' }}>
              {method === 'cash' ? 'Cash' : 'Overschrijving'} · {koperNaam || '-'}
            </div>
          )}
        </div>
        ...
        <button
          ...
          style={{
            width: '100%',
            background: canSubmit ? '#c0392b' : '#555',
            border: 'none',
            color: '#fff',
            padding: '18px',
            borderRadius: '12px',
      ...
      <div>
        ...
        {cart.length === 0 ? (
          ...
          <>
                ...
                <div key={item.id} style={{ background: '#2d2d2d', borderRadius: '12px', padding: '14px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <button
                      ...
                      style={{ background: '#1a1a1a', border: '1px solid #3a3a3a', color: '#fff', width: '32px', height: '32px', borderRadius: '8px', cursor: 'pointer', fontSize: '18px' }}
                    >
                      −
                    </button>
                  ...
                  <div style={{ fontWeight: '800', minWidth: '70px', textAlign: 'right' }}>
                    {fmtBedrag(item.price * item.qty)}
                  </div>

                  <button
                    onClick={() => removeItem(item.id)}
                    style={{ background: 'none', border: 'none', color: '#e74c3c', cursor: 'pointer', fontSize: '22px', padding: '4px', lineHeight: 1 }}
                  >
                    ✕
                  </button>
                </div>
            ...
            <div style={{ fontSize: '22px', fontWeight: '800', textAlign: 'right', marginBottom: '14px' }}>
        ...
        {CATS.map(c => (
          <button
            ...
            style={{
              ...
              padding: '0 20px',
              borderRadius: '22px',
              border: cat === c ? 'none' : '1px solid #3a3a3a',
              background: cat === c ? '#c0392b' : '#2d2d2d',
              color: '#fff',
              fontSize: '15px',
      ...
      {overlay && (
        ...
        >
          <div
            ...
            style={{ background: '#2d2d2d', borderRadius: '16px', padding: '22px', width: '100%', maxWidth: '360px', textAlign: 'center' }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              ...
              <button
                onClick={() => setOverlay(null)}
                style={{ background: 'none', border: 'none', color: '#aaa', fontSize: '26px', cursor: 'pointer', lineHeight: 1, padding: '0 4px' }}
              >
                ✕
              </button>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', margin: '12px 0' }}>
              <ProductIcon product={overlay} size={54} />
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
              {overlay.tweedehands && (
                <span style={{ background: '#d4a017', color: '#1a1000', borderRadius: '6px', padding: '4px 10px', fontSize: '12px', fontWeight: '800' }}>
                  2e hands
                </span>
              )}

              {((overlay.variant || '').includes('Ladies Only') || (overlay.name || '').includes('Ladies Only')) && (
                <span style={{ background: '#8e44ad', color: '#fff', borderRadius: '6px', padding: '4px 10px', fontSize: '12px', fontWeight: '800' }}>
                  Ladies Only
                </span>
              )}
              ...
              <span
                style={{
                  background: (overlay.stock || 0) < 5 ? 'rgba(243,156,18,0.15)' : 'rgba(39,174,96,0.15)',
                  border: `1px solid ${(overlay.stock || 0) < 5 ? '#f39c12' : '#27ae60'}`,
                  borderRadius: '6px',
                  padding: '4px 10px',
                  fontSize: '12px',
                  color: (overlay.stock || 0) < 5 ? '#f39c12' : '#27ae60',
                }}
              >
            ...
            <button
              ...
              style={{
                width: '100%',
                background: (overlay.price || 0) === 0 ? '#555' : '#c0392b',
                border: 'none',
                color: '#fff',
                padding: '18px',
                borderRadius: '12px',
                fontSize: '18px',
                fontWeight: '700',
                cursor: (overlay.price || 0) === 0 ? 'not-allowed' : 'pointer',
              }}
            >
      ...
      {cartCount > 0 && (
        <div style={{ position: 'fixed', left: '16px', right: '16px', bottom: '16px', background: '#c0392b', borderRadius: '14px', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: '12px', boxShadow: '0 8px 22px rgba(0,0,0,0.35)', zIndex: 50 }}>
          <button
            ...
            style={{ flex: 1, background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontWeight: '700', fontSize: '16px', textAlign: 'left' }}
          >
            Winkelkar · {cartCount} item{cartCount !== 1 ? 's' : ''}
          </button>
