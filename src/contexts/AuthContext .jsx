...
// Config-data (categorieën, gordels, ...) verandert zelden. Kost primeert hier:
// de belangrijkste vereiste ("beheerder ziet eigen wijziging meteen") wordt
// afgedekt door refreshConfigCache() die expliciet aangeroepen wordt vanuit
// CrudLijstBeheer/GroepenBeheer na een schrijfactie — dat kost maar reads bij
// een effectieve wijziging, niet bij elke app-open. Dit TTL/FRESH-venster is
// enkel de vangnet-laag voor ándere, al open sessies (andere leden/tabbladen)
// die de wijziging nog niet via hun eigen invalidatie meekregen:
//   - jonger dan CONFIG_CACHE_FRESH_MS  → cache tonen, GEEN extra reads.
//   - tussen FRESH en TTL               → cache tonen + op de achtergrond verversen.
//   - ouder dan TTL, of geen cache      → blokkerend vers laden (zoals voorheen).
// Let op: sessionStorage is per tab/browsertab, dus bij een PWA die vanaf het
// beginscherm herstart telt elke herstart als een nieuwe sessie — de kost
// schaalt dus met "aantal keer app geopend/herladen per toestel", niet met
// een vaste tijdsinterval over alle gebruikers heen. Ruimere waarden hier
// verlagen dus de kost het meest bij frequent herladen binnen dezelfde tab.
const CONFIG_CACHE_KEY = 'configCache';
const CONFIG_CACHE_FRESH_MS = 4 * 60 * 60 * 1000; // 4 uur — binnen dit venster: geen extra read
const CONFIG_CACHE_TTL_MS = 8 * 60 * 60 * 1000; // 8 uur — hard veiligheidsnet
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // Cache uit sessionStorage tonen indien beschikbaar → geen blokkerende
    // reads bij app-herstart/tabwissel. Binnen CONFIG_CACHE_FRESH_MS stoppen
    // we hier (geen extra reads); daarna tonen we de cache wel maar laden we
    // ook op de achtergrond vers (stale-while-revalidate).
    try {
      const raw = sessionStorage.getItem(CONFIG_CACHE_KEY);
      if (raw) {
        const parsed = JSON.parse(raw);
        const leeftijd = parsed?.ts ? Date.now() - parsed.ts : Infinity;
        if (parsed?.uid === firebaseUser.uid && parsed.data && leeftijd < CONFIG_CACHE_TTL_MS) {
          setConfigCache(parsed.data);
          if (leeftijd < CONFIG_CACHE_FRESH_MS) {
            return () => { actief = false; };
          }
        }
      }
